...
class Session:
	...
	def client_registered(self, client):
		...
		if self.registered_clients.size() == 0 and client.c_ip == self.host_ip:
			self.host_port = client.c_port
		...
		for addressed_client in self.registered_clients:
			...
			for client in self.registered_clients:
				if not client.name == addressed_client.name:
					address_list.append(client.name + ":" + address_to_string((client.ip, client.port))+":"+str(self.host_ip==client.ip and self.host_port==client.port)+":"+address_to_string((client.local_ip, client.port)))
